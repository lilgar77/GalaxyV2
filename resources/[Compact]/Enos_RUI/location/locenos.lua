                ...
                if IsControlJustPressed(1, 38) then
                    ...
                    while IsLocMenuOpen do
                        ...
                        RageUI.IsVisible(RMenu:Get('location', 'main'), true, true, true, function()
                            ...
                            RageUI.ButtonWithStyle("Golf7", " ", { RightLabel = "~h~~g~500$" }, true, function(_, Active, Selected)
                                if Active and SelectedCarPrev ~= 1 then
                                    SelectedCarPrev = 1
                                    if DoesEntityExist(CarPrev) then
                                        DeleteEntity(CarPrev)
                                    end
                                    while not HasModelLoaded(GetHashKey("golf7")) do
                                        RequestModel(GetHashKey("golf7"))
                                        Wait(100)
                                    end
                                    CarPrev = CreateVehicle(GetHashKey("golf7"), -1047.62, -2669.05, 14.570, 321.0, false)
                                end
                                if Selected then
                                            if DoesEntityExist(CarPrev) then
                                                DeleteEntity(CarPrev)
                                            ...
                                            local FinalCar = CreateVehicle(GetHashKey("golf7"), -1047.62, -2669.05, 14.570, 321.0, true)
                                            RageUI.Popup({
                                                colors = 140,
                                                message = "Véhicule de location sortie. Soit prudent sur la route. (-500$)",
                                            })
