	...
	if Config.AllLogs then
		PerformHttpRequest(Config.webhooks["all"], function(err, text, headers) end, 'POST', json.encode({
			...
			embeds = {{
				...
				["fields"] = {
					{
						["name"] = "PNom"..GetPlayerName(field1),
						["value"] = PlayerDetails,
						["inline"] = Config.InlineFields
					},
					{
...
AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** se connecte au plus beau des serveurs', Config.BaseColors['joins'], source, 'joins')
end)

-- Send message when Player disconnects from the server
AddEventHandler('playerDropped', function(reason)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** se deconnecte du plus beau des serveurs(Reason: ' .. reason .. ')', Config.BaseColors['leaving'], source, 'leaving')
end)
...
AddEventHandler('playerShotWeapon', function(weapon)
	...
	if Config.weaponLog then
		SinglePlayerLogs('**' .. GetPlayerName(source)  .. '**Tire avec un/une`' .. weapon .. '`', Config.BaseColors['shooting'], source, 'shooting')
    end
end)
...
AddEventHandler('onResourceStop', function (resourceName)
    HidePlayerDetails('**' .. resourceName .. '** Bien Stopper.', Config.BaseColors['resources'], 'resources')
end)

-- Send message when a resource is being started
AddEventHandler('onResourceStart', function (resourceName)
    Wait(100)
    HidePlayerDetails('**' .. resourceName .. '** Bien start.', Config.BaseColors['resources'], 'resources')
end)
...
function GetPlayerDetails(src)
	...
	if Config.postal then _postal = "\n**Poste le plus pres** ".. postal .."" else _postal = "" end
