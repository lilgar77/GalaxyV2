...
local function openBlanchiement()
	RMenu.Add("zj_blanchi", "zj_blanchi_main", RageUI.CreateMenu("~y~Blanchiment","Pourcentage : ~r~25%"))
...
local Blanchir = {
	LOC = {
		{position = vector3(4447.9, -4444.88, 7.24)}
	}
}
...
Citizen.CreateThread(function()
	...
	ped = CreatePed("0x26A562B7", hash, 4447.9, -4444.88, 6.24, 89.6566, false, true)
    ...
    while true do
        ...
        for k,v in pairs(Blanchir.LOC) do
            if #(PlyCoord - v.position) < 1.5 then
                RageUI.Text({ message = "Appuyez sur [~y~E~s~] pour parler à Tonton!", time_display = 1 })
