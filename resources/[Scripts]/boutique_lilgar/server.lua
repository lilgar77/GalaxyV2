...
ESX.RegisterServerCallback('Boutique:DonnePoint', function(source, cb, point, boutique_id)
   ...
   MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
        --print(result[1].galaxycoin)
        if result[1].galaxycoin >= tonumber(point) then
            local newpoint = result[1].galaxycoin - tonumber(point)
            MySQL.Async.execute("UPDATE `users` SET `galaxycoin`= '".. newpoint .."' WHERE `identifier` = '".. xPlayer.getIdentifier() .."'", {}, function() end)
            MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `boutique_id` = '".. boutique_id .."'", {}, function (result2)
                local addpoint = result2[1].galaxycoin + tonumber(point)
                ...
                MySQL.Async.execute("UPDATE `users` SET `galaxycoin`= '".. addpoint .."' WHERE `boutique_id` = '".. boutique_id .."'", {}, function() end)
            end)
            cb(true)
        else
            cb(false)
...
AddEventHandler('boutique:deltniop', function(point)
	...
	MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier=@identifier', {
        ...
        MySQL.Async.execute('UPDATE `users` SET `galaxycoin`=@point  WHERE identifier=@identifier', {
            ['@identifier'] = license,
            ['@point'] = npoint 
        }, function(rowsChange)
        end)
